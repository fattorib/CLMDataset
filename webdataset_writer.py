...
if __name__ == "__main__":
    ...
    with wds.ShardWriter(
        f"data/processed/sharded/{CORPUS_PRFIX}_train-%06d.tar.gz",
        maxsize=int(1e9),
        maxcount=int(1e5),
    ) as sink:
        for chunk in tqdm(range(0, 100)):
            data, shape = open_chunk(
                filename=f"{CORPUS_PRFIX}_train", chunk_idx=chunk, N_CTX=N_CTX
            )
    ...
    index_counter = 0
    with wds.ShardWriter(
        f"data/processed/sharded/{CORPUS_PRFIX}_val-%06d.tar.gz",
        maxsize=int(1e9),
        maxcount=int(1e5),
    ) as sink:
        for chunk in tqdm(range(0, 10)):
            data, shape = open_chunk(
                filename=f"{CORPUS_PRFIX}_val", chunk_idx=chunk, N_CTX=N_CTX
            )
            for i, index in enumerate(data):
                sample = {
                    "__key__": f"sample_{i+index_counter}",
